-- Migration: 052a_fix_seed_migration.sql
-- Description: Fix seed migration by removing dependency on migration_log table
-- Date: 2025-09-08
-- Author: Claude

-- This is the same as 052 but without the migration_log insert
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 052a completed successfully';
  RAISE NOTICE 'Extraction rules created: %', (
    SELECT COUNT(*) 
    FROM public.brand_extraction_rules 
    WHERE created_at >= NOW() - INTERVAL '5 minutes'
  );
  RAISE NOTICE 'Brands updated: %', (
    SELECT COUNT(*) 
    FROM public.brands 
