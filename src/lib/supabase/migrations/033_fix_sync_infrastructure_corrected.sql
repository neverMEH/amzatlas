...
VALUES 
    ('sqp', 'asin_performance_data', 24, true),
    ('sqp', 'search_query_performance', 24, true),
    ('sqp', 'daily_sqp_data', 24, true)
...
-- Grant sequence permissions (try without checking if they exist first)
DO $$
BEGIN
    GRANT USAGE ON SEQUENCE sqp.refresh_config_id_seq TO service_role;
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE 'refresh_config_id_seq does not exist yet';
END $$;
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE 'refresh_audit_log_id_seq does not exist yet';
END $$;
...
BEGIN
    ...
    -- Show current configurations with proper RECORD variable
    ...
    LOOP
        RAISE NOTICE '  %.%: % hours, enabled: %, last: %', 
            config_record.table_schema, 
            config_record.table_name, 
            config_record.refresh_interval_hours,
            config_record.is_enabled,
            COALESCE(config_record.last_refresh_at::TEXT, 'never');
    END LOOP;
