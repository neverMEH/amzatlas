-- Migration: Add automatic brand matching functionality
-- Description: Create functions and triggers for automatic ASIN-to-brand matching
...
-- This function will be called by a trigger on sqp.asin_performance_data
...
-- Make sure the function has access to public schema tables
GRANT USAGE ON SCHEMA public TO postgres;
GRANT SELECT ON public.brands TO postgres;
GRANT INSERT, UPDATE ON public.asin_brand_mapping TO postgres;

-- Create the trigger on the actual table, not the view
DROP TRIGGER IF EXISTS auto_match_brand_trigger ON sqp.asin_performance_data;
CREATE TRIGGER auto_match_brand_trigger
AFTER INSERT OR UPDATE ON sqp.asin_performance_data
FOR EACH ROW
EXECUTE FUNCTION public.auto_match_brand_on_title_update();

-- Add indexes for better performance on the actual tables
CREATE INDEX IF NOT EXISTS idx_sqp_asin_performance_product_title 
ON sqp.asin_performance_data(product_title);

CREATE INDEX IF NOT EXISTS idx_public_brands_brand_name 
